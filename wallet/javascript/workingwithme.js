...
currentPrice.open("GET", "https://bitpay.com/api/rates", true);
//currentPrice.onreadystatechange = function () {
// if (this.readyState == 4 && this.status == 200) {
//alert(this.responseText);
//    JSON.parse(console.log(currentPrice.responseText));
// }
//if (currentPrice.readyState == 4) {
//  var response = currentPrice.responseText;
//  JSON.parse(console.log("ok" + response));
...
//}
//};
currentPrice.onload = function () {
  // Begin accessing JSON data here
  var ticker = JSON.parse(currentPrice.responseText);
  var price = ticker[2].rate;
  document.getElementById("btc").innerHTML = "1 BTC = " + "$" + price;
};
currentPrice.setRequestHeader("Content-type", "application/json");
currentPrice.send();
...
function broadcast_cb(res) {
  if (res === "") {
    ...
    $("amount").value = "";
    $("status").innerHTML =
      '<a target="_blank" href="https://blockchain.info/tx/' +
      tx.txid() +
  ...
  if (utxo !== false) {
    ...
    if (total >= tx_total) {
      ...
      err =
        ...
        js.format_money(tx_total - total, 8) +
        " BTC more.";
...
if (!JSON || !JSON.parse)
  ...
  if ("withCredentials" in x) {
    ...
    passphrase.addEventListener("input", function () {
      ...
      days_to_crack.style.color =
        daysToCrack > 7 ? (days_to_crack < 30 ? "#AF7817" : "green") : "#c03";
    });
  } else
    alert(
      "Your browser doesn't support Cross-Origin Resource Sharing. This page will not work, sorry."
    );
}
